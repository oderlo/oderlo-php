<?php
/**
 * Send request to Oderlo API
 * Better don't use this class directly, use Oderlo_VtWeb, Oderlo_VtDirect, Oderlo_Transaction
 */

class Oderlo_ApiRequestor {
	...
	public static function remoteCall($url, $server_key, $data_hash, $post = true){
		...
		// merging with Oderlo_Config::$curlOptions
		if (count(Oderlo_Config::$curlOptions)) {
			// We need to combine headers manually, because it's array and it will no be merged
			if (Oderlo_Config::$curlOptions[CURLOPT_HTTPHEADER]) {
				$mergedHeders = array_merge($curl_options[CURLOPT_HTTPHEADER], Oderlo_Config::$curlOptions[CURLOPT_HTTPHEADER]);
				$headerOptions = array( CURLOPT_HTTPHEADER => $mergedHeders );
			} else {
				$mergedHeders = array();
			}

			$curl_options = array_replace_recursive($curl_options, Oderlo_Config::$curlOptions, $headerOptions);
		}
		...
		if ($result === FALSE) {
			...
			if (!in_array($result_array->status_code, array(200, 201, 202, 407))) {
				$message = 'Oderlo Error (' . $result_array->status_code . '): '
